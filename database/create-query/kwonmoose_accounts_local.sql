DROP SCHEMA IF EXISTS `kwonmoose_accounts_local`;
CREATE SCHEMA `kwonmoose_accounts_local`;
USE `kwonmoose_accounts_local`;


# region 회원 테이블
DROP TABLE IF EXISTS `kwonmoose_accounts_local`.`user`;
CREATE TABLE `kwonmoose_accounts_local`.`user`
(
...
create user 'kwonmoose_local'@'%' identified by 'kwonmoose_local_1234';
grant all privileges on kwonmoose_accounts_local.* to 'kwonmoose_local'@'%';